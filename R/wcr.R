wcr <- function(fmlf, dat, param, B = 999, cluster){
  ...
  # fmlf <- as.formula(fmlf)
  # fmlf <- as.formula('math ~ male + mhmwk + mses + ses')
  # fmlr <- 'math ~ mhmwk + mses + ses'
  ...
  ystar <- pred_null + utilde * t(S) %*% v
  numer_all <- invXX %*% crossprod(X, ystar)
  ...
  # bm <- matrix(b, ncol = B, nrow = length(b))
  # (invXX %*% t(X)) %*% (utilde * t(S) %*% v)
  ...
  boot_se <- vapply(1:B, function(b) {
    ...
    scores_b <- rowsum(X * ustarhat[, b], gr) # G x k matrix

    meat_b <- crossprod(scores_b)             # k x k matrix
    vcov_b <- invXX %*% meat_b %*% invXX * adj
    sqrt(diag(vcov_b))
